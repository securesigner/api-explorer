...
# Pre-commit
cp "$HOOKS_SRC/pre-commit-api-validate.sh" "$HOOKS_DST/pre-commit"
chmod +x "$HOOKS_DST/pre-commit"
echo "  ✅ pre-commit  → JSON validation & formatting"

# Commit-msg
cp "$HOOKS_SRC/commit-msg-api-convention.sh" "$HOOKS_DST/commit-msg"
chmod +x "$HOOKS_DST/commit-msg"
echo "  ✅ commit-msg   → Conventional commit enforcement"

# Pre-push
cp "$HOOKS_SRC/pre-push-api-healthcheck.sh" "$HOOKS_DST/pre-push"
